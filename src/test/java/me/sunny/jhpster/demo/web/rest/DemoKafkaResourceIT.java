...
import me.sunny.jhpster.demo.config.KafkaProperties;
...
class DemoKafkaResourceIT {
    ...
    void setup() {
        ...
        DemoKafkaResource kafkaResource = new DemoKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/demo-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/demo-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
